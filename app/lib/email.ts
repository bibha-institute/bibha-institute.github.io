...
export async function sendInterestEmails(signup: { name: string; email: string; role: string; location: string; interests: string; contribution: string }) {
  ...
  const confirmation = await sendEmail({
    to: signup.email,
    subject: "Your interest in the BAIRE founding network is registered",
    html: `<div style="font-family:Arial,sans-serif;max-width:620px;line-height:1.6;color:#123f34"><h1 style="font-family:Georgia,serif;font-weight:400">Thank you, ${safe.name}.</h1><p>Your interest in the BAIRE Research Commons founding network has been saved.</p><p>This is an early no-clinical-data network pilot—not yet a member account or project acceptance. We will review founding-network responses and contact suitable contributors as the pilot takes shape.</p><p><strong>Your selected role:</strong> ${safe.role}<br><strong>Research interests:</strong> ${safe.interests}</p><p>BAIRE Research Commons<br><small>Research talent is everywhere. Opportunity should be too.</small></p></div>`,
  });
  const owner = founderEmail ? await sendEmail({
    to: founderEmail,
    replyTo: signup.email,
    subject: `New BAIRE founding-network registration — ${signup.name}`,
...
export async function sendFeedbackNotification(feedback: { audience: string; rating: number; message: string; email: string }) {
  ...
  return sendEmail({
    ...
    subject: `New BAIRE feedback — ${feedback.rating}/5`,
